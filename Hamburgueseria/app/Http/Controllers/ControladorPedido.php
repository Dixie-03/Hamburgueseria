...
class ControladorPedido extends Controller
{
    ...
    public function cargarGrilla()
    {
        ...
        for ($i = $inicio; $i < count($aPedidos) && $cont < $registros_por_pagina; $i++) {
            ...
            if ($aPedidos[$i]->estado == "Cancelado") :
                $row[] = '<span style="color:#D82323; font-weight: bold;">' .  $aPedidos[$i]->estado . '</span>';
            elseif ($aPedidos[$i]->estado == "En preparación") :
                $row[] = '<span style="color:#F3B532; font-weight: bold;">' .  $aPedidos[$i]->estado . '</span>';
            elseif ($aPedidos[$i]->estado == "Entregado") :
                $row[] = '<span style="color:#06B77C; font-weight: bold;">' .  $aPedidos[$i]->estado . '</span>';
            elseif ($aPedidos[$i]->estado == "Pendiente") :
                $row[] = '<span style="color:#8D4B3D; font-weight: bold;">' .  $aPedidos[$i]->estado . '</span>';
            endif;
            $cont++;
            $data[] = $row;
        }
